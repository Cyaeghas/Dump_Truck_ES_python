...
Wt = 0 #正在称重的卡车数量 0，1
max_Loader_num = 2 #装载车数量
max_Scale_num = 1 #称重台数量

#仿真时钟
Clock = 0
#卡车容器
Truck = [0 for i in range(10)]
#创建truck类
class truck:
...
#打印报告
def print_report():
    LoaderUtil  = BL/2/StopSimulationTime
    ScaleUtil   = BS/StopSimulationTime
    print()
...
#初始化        
def Initialization():
    global Lt,Wt,MLQ,MWQ,BL,BS,T,LoaderQueue,ScaleQueue,FutureEventList
    #仿真时钟初始化
    Clock = 0
    #统计量初始化
    T = 0 #卡车在系统内停留时间
...
if __name__ == '__main__':
    #进行50次仿真
    n = 50
    begin_one = repeat_simu(n,6)
    print('6辆卡车平均停留时间为：%f'%begin_one)
    #改变概率条件
    TravelTimeProb = [0.1,0.3,0.6,1]
    #题一条件
    max_Loader_num_0 = max_Loader_num
    t_1 = t_2 = t = repeat_simu(n,6)
    print('概率改变后')
    print('采用原方案，6辆卡车卡车平均停留时间为：%f'%t)
    print('题目一：')
    while t_1>=t and max_Loader_num<100:
        max_Loader_num += 1
        t_1 = repeat_simu(n,6)
    print('增加 %d 辆装载车后，6辆卡车卡车平均停留时间为：%f'%(max_Loader_num-max_Loader_num_0,t_1))
    #探究
    max_Scale_num_0,max_Loader_num = max_Scale_num,max_Loader_num_0
    while t_2>=t and max_Scale_num<100:
        max_Scale_num += 1 
        t_2 = repeat_simu(n,6)
    print('增加 %d 个称重台后，6辆卡车卡车平均停留时间为：%f'%(max_Scale_num-max_Scale_num_0,t_2))
    ...
    while 1:
        #增加第p辆卡车
        Truck[p] = truck(p,Clock,Event[1],Clock)
        LoaderQueue.append(Truck[p])
        t_p = repeat_simu(n,p)
        print('%d 辆卡车时，卡车平均停留时间为：%f'%(p,t_p))
        if t_p<t:
            print('需要增加 %d 辆拉煤车'%(p-6))
            break
        p += 1
